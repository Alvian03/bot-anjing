	...
	client.on('message-new', async (mek) => {
		try {
		    ...
		    case 'owner':
				...
				case 'hidetag':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('Lu saha anying?')
				...
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('Maaf textnya kebanyakan')
					: gtts.save(ranm, dtt, function() {
						exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
							fs.unlinkSync(ranm)
							buff = fs.readFileSync(rano)
							if (err) return reply('Gagal slurr:(')
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				case 'donate':
					...
					break
                case 'tes':
                   client.sendMessage(from, 'Ada syng ada :)', text, {quoted: mek})
                case 'ttp':
              ...
              case 'abangjago':
              case 'abgjago':
                    let eerrr = fs.readFileSync('./lib/abangjago.mp3')
