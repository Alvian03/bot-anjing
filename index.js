...
│ *SEMOGA BETAH!!* `
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["62895414533737@s.whatsapp.net"] // replace this with your number
			const adminbotnumber = ["62895414533737@s.whatsapp.net"]
			const frendsowner = ["62895414533737@s.whatsapp.net"]
			...
			case 'creator':
                 ...
                 client.sendMessage(from, 'Noh nmr owner,,jan lupa sv  ><',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
				...
				case 'hidetag':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply('*Lu saha?*')
                ...
                case 'quotemaker':
					...
					break
				case 'galaxtext':
					if (args.length < 1) return reply('mau apa tod?')
					teks = body.slice(12)
				...
				case 'tts':
					...
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('Idih-idih ngelunjak, textnya kebanyakan')
               ...
               case 'infogempa':
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘 BABUBOT 〙', members_id, true)
					break
				case 'clearall':
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('Yang mau di add saha? jin ya?')
