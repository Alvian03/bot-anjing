...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Halo @${num.split('@')[0]}\nSelamat datang di group _*${mdata.subject}*_ ✰ Moga betah yak ❛New Member Intro`
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const frendsowner = ["6282268476590@s.whatsapp.net"]
			...
			case 'creator':
                 ...
                 client.sendMessage(from, 'Tuh nmr owner, jan lupa sv ><',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
                ...
                case 'quotemaker':
					...
					break
				case 'galaxtext':
					if (args.length < 1) return reply('mau apa')
					teks = body.slice(12)
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
				...
				case 'ssweb':
					...
					break
				case 'walpaperhd':
					if (args.length < 1) return reply('teks nya mana bos')
					teks = body.slice(7)
					reply(mess.wait)
